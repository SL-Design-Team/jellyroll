      ...
      items: [
        ...
        },
        {
          ...
          meta: {
            ...
            options: "Plain (no affix), unit suffix, or prefix — the stepper is independent and available with any of them. Integer or decimal precision. Min/max/step bounds. Width is fluid by default; add `--fit` with `--digits` set to the character count of the longest legal value when the field should be no wider than the values it can hold.",
            usage: "Use for bounded numeric values — counts, timeouts, sizes. Use with a unit suffix when the unit clarifies meaning. For unbounded text-like numerics (phone, ID), use a plain Input. **Stepper rule:** show the stepper when this field is the only control for its value; omit it when something else in the same row already changes that value — a paired Slider, for instance — or when the field is `readonly`. One value never gets two increment affordances. Composing this field inside another component means using it as-is and switching off the parts the host already provides; never re-create the field to change its size, font, or affix treatment.",
            behaviors: "The browser's own `type=number` spin buttons are always suppressed — the component draws its own stepper, and the native ones otherwise sit on top of the value and clip it. Arrow up/down increments by step; Shift+arrow by 10× step. Steppers respect min/max bounds and disable at the limits. Pasting non-numeric input is rejected silently. On blur, a value outside min/max snaps to the nearest bound and the field surfaces its own inline error — it does not rewrite the value while the user is still typing. Decimal separator matches the user's locale."
          }
        },
        {
        ...
        },
        {
          ...
          meta: {
            anatomy: "Label row (label left, min–max range right) · single-thumb horizontal track — 3px track, 16px circular thumb, 28px visual height · optional value readout or a paired Number input on the right — the canonical component, not a lookalike · optional help/error text below. Filled portion Blue-600; unfilled track Blue-200. Disabled uses Grey-200 track with Grey-400 fill/thumb.",
            options: "Widths: fluid (default), sm 160px, md 240px, lg 320px. Value display: none, read-only readout, or a paired Number input (stepper omitted — the slider already increments) sized off `--digits` so the field is never wider than the slider's longest legal value. Optional label, min–max range caption, unit suffix, and help text. No tick marks, value bubble, vertical orientation, or dual-thumb range in the base component.",
        ...
        },
        {
          ...
          meta: {
            anatomy: "A ring masked out of a conic gradient that ramps transparent → `currentColor` over 360°, with a round head dot capping the seam so the comet reads as one tapered tail. Ring thickness via `--sl-spin-thick`; rotation 1.1s linear infinite, slowing to 2.8s under `prefers-reduced-motion`.",
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          guidelinesInPreview: true,
          name: "Drawer",
          tagline: "Side panel that slides in from the right for details or secondary tasks.",
          meta: {
            anatomy: "Two types: overlay drawer (floats over the page with a scrim) and push drawer (a column beside the page; the page reflows). Both: panel slides from right · 56px header with title and close `×` · scrolling body padded 20/24 · 64px footer (required on overlay, optional on push).",
            options: "Overlay drawer: modal, scrim, fixed widths 480 / 560 (default) / 720. Push drawer: non-modal, no scrim, 1px Grey-300 left border, resizable 320–720 (default 480). Never float a panel over content without a scrim.",
            usage: "Work down this test and stop at the first rule that applies: (1) the user needs the page and the drawer at the same time — push; (2) it is one task that must be finished or abandoned, with a Save/Create/Continue action — overlay; (3) opened by selecting an object on the page — push, opened from a toolbar or menu to start something new — overlay; (4) they would keep it open across several actions — push; (5) still unsure: read-mostly content — push, a form that commits — overlay. Tiebreakers: content wider than 720 or a two-column body — overlay; workspace under 960 — overlay; when both fit, choose push. A push drawer may open an overlay drawer or modal on top, but never stack two overlay drawers. For confirms use a Modal; for temporary notifications use a Toast.",
            behaviors: "Slides in 240ms. Overlay drawer: scrim fades 200ms, blocks the page, traps focus (`role=\"dialog\" aria-modal=\"true\"`), closes on X / Esc / scrim click. Push drawer: the page reflows narrower and stays fully interactive, no focus trap (`role=\"region\"`), outside click does not close, selecting another object swaps content in place. Push drawers resize via an 8px left-edge handle (`role=\"separator\"`, arrow keys ±16px) showing a 2px Blue-600 line on hover/drag and Teal-600 on keyboard focus. Under a 960px workspace a push drawer opens as an overlay drawer."
          }
        },
        {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          tagline: "Guided multi-step flow with a vertical step rail.",
          meta: {
            anatomy: "Eight elements: (1) header with an action-led title and close `×` · (2) 240px vertical step rail · (3) completed step — Blue-600 fill with a 16px white check, clickable · (4) current step — white with a 2px Blue-600 ring, `aria-current=\"step\"` · (5) upcoming step — white with a 1.5px Grey-400 border, `aria-disabled` · (6) step title — a question or instruction expanding the label · (7) scrolling body · (8) footer with `Cancel` (ghost, left) and `Back` / `Next` (right), one primary. Rail: 22px dots with a 14px check, Blue-50 current row, connectors that turn Blue-600 behind completed steps. Labels 13/20 ExtraBold with an optional 11px sub-label.",
            options: "Every wizard uses the vertical step rail. Modal wizard for shorter flows, or in-page wizard for longer flows and steps with substantial content (title in the page header). Optional steps are marked `(optional)`; when only optional steps remain, a tertiary `Skip to review` sits beside Back. `Save draft` in the header for wizards of 5+ steps.",
            usage: "Use for a long task that breaks into 3+ ordered steps where later choices depend on earlier ones, or first-time setup that needs guidance (AutoSync setup, creating a Task, connection setup). Don't use for fewer than 3 steps (single form), non-linear editing (form sections with Save), or simple data entry. Always end with a Review step listing every input as label/value pairs grouped by step with `Edit` links. The final primary names the result (`Create task`, `Start sync`) — never Finish or Submit. Wizard title is verb + object; step labels are 1–2 word nouns; the last step is labelled Review.",
            behaviors: "`Next` validates the current step and stays disabled until it does — never auto-advance. `Back` keeps all values and is disabled on the first step; going back never resets later steps. Clicking a completed step returns to it (hover: dot to Blue-700, plus Grey-100 row fill on the vertical rail; focus: `--shadow-focus-ring` on the whole step); going back keeps later steps completed so the user can jump straight back; jumping forward to a step not yet reached is not allowed. Cancel or close with unsaved input asks to confirm. `Save draft` persists 14 days and resumes at the last completed step. Long-running submits show a progress bar and status message in the body with the footer disabled; completion shows a success or error message with one primary action and Close, and errors keep the input with `Back to review`. The indicator is a `nav` labelled `Steps`; completed steps are buttons; on step change focus moves to the new step title; validation errors move focus to the first invalid field."
          }
        }
      ]
    },
