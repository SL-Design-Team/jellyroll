      ...
      items: [
        ...
        },
        {
          ...
          meta: {
            ...
            usage: "Pick an overlay drawer for one focused task the user starts, finishes and closes (build pipeline, configure a Snap, account settings). Pick a push drawer when the user needs the drawer and the page at the same time — details about a selected object, or a reference that stays open (run details, lint results, patterns catalog, MCP servers). For confirms, use a Modal. For temporary notifications, use a Toast.",
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          meta: {
            anatomy: "Eight elements: (1) header with an action-led title and close `×` · (2) 240px vertical step rail (the default) · (3) completed step — Blue-600 fill with a 16px white check, clickable · (4) current step — white with a 2px Blue-600 ring, `aria-current=\"step\"` · (5) upcoming step — white with a 1.5px Grey-400 border, `aria-disabled` · (6) step title — a question or instruction expanding the label · (7) scrolling body · (8) footer with `Cancel` (ghost, left) and `Back` / `Next` (right), one primary. Rail: 22px dots with a 14px check, Blue-50 current row, connectors that turn Blue-600 behind completed steps. Exception: a horizontal indicator (24px dots, 2px connectors) only when a modal is too narrow for the rail, for 3–5 steps. Labels 13/20 ExtraBold with an optional 11px sub-label.",
            options: "The vertical step rail is the default. Modal wizard for shorter flows, or in-page wizard for longer flows and steps with substantial content (title in the page header). Horizontal indicator only as an exception, when a modal is too narrow for the rail and the flow has 3–5 steps. Optional steps are marked `(optional)`; when only optional steps remain, a tertiary `Skip to review` sits beside Back. `Save draft` in the header for wizards of 5+ steps.",
